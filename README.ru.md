...
Публичная бета, текущая версия 0.38.0. Issues и фидбек приветствуются. Нативное macOS-приложение — в [app/](app/), сборка `app/make_app.sh`; компаньон для iPhone — в [app-ios/](app-ios/). Планы — в [ROADMAP.ru.md](ROADMAP.ru.md): локализация интерфейса приложений, прямая доставка записей с телефона по Wi-Fi, просмотрщик графа без Obsidian.
