...
def revise(graph: pathlib.Path, only_names: list[str] | None = None,
    ...
    `may_continue` — ночное окно: зовётся перед каждой парой и вправе ЖДАТЬ (конец
    живой встречи, не дольше конца ночи); False — прогон обрывается со status
    «stopped» и недосмотренными ядрами в `unjudged_names`. Знак обратный `should_stop`
    у `embed_pending`: там мгновенная проверка «пора остановиться», здесь разрешение
    идти дальше. У приложения окно собирает одна дверь — `graphs.revise_cores`
    (Opus, критика круга 1 по коду №365).
    """
    ...
    for позиция, (c, a, b) in enumerate(pairs):
        ...
        # ищет, окно ему передают (№365; контракт — в докстринге)
        if not may_continue():
            out["stopped"] = True
