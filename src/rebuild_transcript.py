...
def rebuild(live: pathlib.Path, cfg: dict) -> pathlib.Path | None:
    ...
    if mic_p is not None:
        ...
        if len(mic) > sr * 20:
            ...
            if owner_label == "Собеседник":
                # Ровно та же проверка, что в живой ленте: коллизия метки
                # владельца с нейтральной. Шире брать нельзя — «Собеседник
                # Иванов» живая лента подписывает спокойно, и финал не
                # должен с ней расходиться (ревью 19.08, второй круг).
                # Иначе метка увела бы сегменты удалённого собеседника в
                # микрофонную дорожку, и текст испортился бы молча.
                owner_voice = None
            mapping = {}
            for s, e, k in mic_segs:
                if k not in mapping:
