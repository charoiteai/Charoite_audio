...
def rebuild(live: pathlib.Path, cfg: dict) -> pathlib.Path | None:
    ...
    if mic_p is not None:
        ...
        if len(mic) > sr * 20:
            ...
            if owner_label.startswith("Собеседник"):
                # Метка коллизирует с нейтральными: живая лента в этом случае
                # молчит, и офлайн обязан молчать тоже — иначе метка
                # удалённого собеседника уводит его сегменты в микрофонную
                # дорожку и текст портится молча.
                owner_voice = None
            mapping = {}
            for s, e, k in mic_segs:
                if k not in mapping:
