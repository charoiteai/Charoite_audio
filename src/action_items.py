...
def _to_checkbox(line: str) -> str:
    ...
    # Парность не восстановилась (имя длиннее лимита, звёздочки в середине):
    # висячие ** в живом документе хуже, чем нежирное имя, — снимаем все
    # (Minor DS по #462: «…40 символов…** — задача» оставался с хвостом).
    if not body.startswith("**") and body.count("**") % 2 == 1:
        body = body.replace("**", "")
