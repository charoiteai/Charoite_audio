...
# Любой другой заголовок — конец раздела: markdown-заголовок, жирный
# «**Решения:**» — или ГОЛАЯ строка-заголовок с двоеточием на конце
# («Открытые вопросы:»), как пишут минутки 31.08: без неё раздел поручений
# не закрывался, и хвостовые списки становились «задачами» в окне.
_OTHER_SECTION = re.compile(
    r"^\s*(?:#{1,6}\s|\*\*[^*]+:\*\*\s*$|[^\s\-*+•#>][^:\n]{0,60}[:：]\s*$)")
# Строка-пункт: маркер списка в начале.
...
def normalize(text: str) -> str:
    ...
    for line in lines:
        ...
        if inside:
            ...
            if _OTHER_SECTION.match(line) and not _BULLET.match(line):
                inside = False
                out.append(line)
                continue
