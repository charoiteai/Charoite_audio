    ...
    for d, p, text in nodes:
        ...
        for alias in aliases:
            ...
            if target is None and person and aw[:1].isupper():
                ...
                # хвост (основы после среза «-ский» — «гель»/«вель», общий
                # хвост «ель», 3), а имя с фамилией суффикса не делят — «Марк»
...
def candidates(text: str, lex: Lexicon, window: int = 30) -> list[str]:
    ...
    for i, w in enumerate(words):
        ...
        for cl, (node, ctx) in lex.context.items():
            ...
            # ✔). Порог 2, не 3: срез «-ский» укорачивает основы («гель»/
            # «вель» у Гельский/Вельский — общий хвост 3), и у настоящих
            # искажений -ских фамилий он может опускаться до 2.
