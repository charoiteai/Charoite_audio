...
def rewrite_file(path: pathlib.Path, transform, what: str, *, errors: str = "strict") -> int:
    ...
    (DS M5). После второй неудачи — LostRace. `errors` — как читать не-UTF-8; с №263
    не-дефолта не осталось ни у одного вызывающего: и перештамповка имён, и
    мост минуток читают СТРОГО, чтобы не записать битый файл обратно с «�».
    Параметр держим на случай файла, который читать иначе нельзя вовсе. Живёт здесь, рядом с
