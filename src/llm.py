...
class LLM:
    def __init__(self, cfg: dict):
        self.owner = (cfg["sufler"].get("user_name") or "пользователя").strip()
        l = cfg["llm"]
    ...
    def instant(self, tail: str, model: str | None = None) -> Iterator[str]:
        ...
        return self.stream(
            ...
            system=(
                f"Ты отвечаешь ЗА {self.owner} на рабочей встрече или собеседовании, его голосом. "
    ...
    def minutes(self, transcript: str) -> Iterator[str]:
        ...
        return self.stream(
            f"Стенограмма встречи (реплики помечены спикерами):\n\n{transcript}\n\n"
            ...
            "## Решения\n(список «- **что решили** — кто внедряет»; если решений не было — так и напиши)\n"
            "## Поручения\n(список «- **Кто** — что — срок», только реально прозвучавшее)\n"
            "## Открытые вопросы\n## Риски\n"
            "Объём — как у хороших минуток: коротко, без пересказа всей дискуссии. "
