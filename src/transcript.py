...
def speech_of(text: str) -> str:
    """Речь встречи — то, из чего собираются производные (минутки, разбор,
    тезисы): без H1 и без хвоста «Ко-мышление» (мысли модели, не сказанное).

    Одно определение на всех писателей: хеш речи в паспорте производной
    (`live_sidecar`) считался от текста С заголовком, а ретитл переписывает
    H1 (`# Встреча <штамп>` → `# Встреча <штамп> — <тема>`) — и после
    первого же ретитла минутки считались собранными по другой речи, что уже
    стоило LLM-вызова на пути правленой стенограммы (Critical DS входного
    круга по №309). Заголовок — не речь; правило живёт здесь, рядом с
    NOTES_HEAD, а не приватными копиями у писателей."""
    body = text.split(NOTES_HEAD, 1)[0]
    if body.startswith("# "):
        body = body.split("\n", 1)[1] if "\n" in body else ""
    # хвостовые переводы строк — не речь: у файла без хвоста тело кончается
    # пустой строкой, у файла с хвостом — нет, и первая же заметка ко-мышления
    # (или след канала, №234) сдвигала бы хеш речи на один «\n»
    return body.rstrip("\n")
