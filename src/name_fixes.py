...
_FIX = re.compile(r"^\*\*(?P<label>[^*\n]+?)\*\*\s*(?:→|->|=>|—>|⇒)\s*\*\*(?P<name>[^*\n]+?)\*\*"
                  ...
                  re.IGNORECASE)
# Разметка, перевод строки — и «�»: ревизия читается с заменой
# нечитаемых байтов, и имя с таким символом — это потерянный байт, а не имя.
# Написать его в заголовки реплик, участников минуток и узел Люди значило бы
# оставить порчу в графе навсегда (№263).
...
def plan(fixes: list[tuple[str, str, str]], headers: set[str], protected: set[str],
    ...
    for label, name, _why in fixes:
        ...
        if label == name:
            ...
            reason = "имя не годится (заглушка, разметка, нечитаемый байт или длина)"
