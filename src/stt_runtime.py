...
# Маркер — одна константа: audio.py собирает из неё текст предупреждения,
# иначе три рукописные копии строки расходились бы молча (GLM r1 по #538).
MIC_ONLY_WARNING = "СОБЕСЕДНИКОВ В ЗАПИСИ НЕ БУДЕТ"
# Потеря канала — одно событие с разным носителем: собеседники, микрофон
# владельца, оба (или единственный канал). Липкий слой в приложении ОДИН,
# поэтому маркеров три, а строка — одна, и пересобирается из полного
# состава потерянных при каждой смене; отбой шлётся только когда потерянных
# не осталось (входной круг DS и GLM по №235, 18.09).
OWNER_MIC_LOST = "ВАШЕГО МИКРОФОНА В ЗАПИСИ НЕТ"
RECORDING_EMPTY = "ЗАПИСЬ ПУСТА"
STICKY_WARNINGS = (MIC_ONLY_WARNING, OWNER_MIC_LOST, RECORDING_EMPTY)
# Отбой липкого предупреждения: канал умер посреди встречи и ожил (сторож
# перезапустил поток) — строка «не будет» стала бы неправдой до конца
# встречи (№232). Демон шлёт `sticky: false`, приложение снимает слой.
MIC_BACK_NOTICE = "СОБЕСЕДНИКИ СНОВА В ЗАПИСИ"
OWNER_MIC_BACK = "ВАШ МИКРОФОН СНОВА В ЗАПИСИ"
STICKY_CLEARS = (MIC_BACK_NOTICE, OWNER_MIC_BACK)
...
def is_recording_failure(status_text: str) -> bool:
    ...
    отстают», и префикс красил 2 из 4 (круг 3, GLM + DeepSeek). Липкое
    предупреждение сюда НЕ подмешивается: «запись идёт, но неполная» — не
    «запись сломалась», а флаг error ещё и гейт «ошибка демона бьёт критикал
    диска» в приложении; красный липкому приложение даёт само (DS I2 и
    критика GLM r1 по #538).
    """
