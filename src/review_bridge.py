...
def _key(item: str) -> str:
    ...
    самого моста: «⚠ не участник» и «(из ревизии)». Свой ключ оставлял цифры даты
    из «📅 2026-10-01», и поручение с полем и без него дописывалось дважды (№366)."""
    text = re.sub(r"⚠[^:]*:", " ", item)           # «⚠ не участник (Имя):» целиком
    for mark in MARKS.values():
        text = text.replace(mark, " ")
    return task_line.key(text)
