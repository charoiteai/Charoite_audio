...
# cloud_effort — разбор встречи, ночные ревизии досье и ядер (opus, не в темпе
# разговора). cloud_live_effort — вызовы в темпе встречи (haiku: нить и ответ
# на вопрос, потолки 60 и 90 с): low — короче размышление, быстрее ответ; для
# haiku CLI принимает уровень и отвечает (проверено 08.09). Без ключа оба
# вызова шли на «auto» из GUI или на max из shell (№214).
# cloud_night_effort — ночные ревизии досье и ядер: потолок 600 с и нет замка
# графа, поэтому глубина важнее скорости — high, как шло из launchd до
# явного ключа; общий с разбором ключ тянул бы ночь за настройкой дебрифа
# (DS r1 по #528).
EFFORT_DEFAULTS = {"cloud_effort": "medium", "cloud_live_effort": "low", "cloud_night_effort": "high"}
