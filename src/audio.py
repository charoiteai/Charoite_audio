...
class AudioHub:
    ...
    def __init__(self, cfg: dict, stamp: str | None = None):
        ...
        # label → [потеря с прошлого отчёта, время отчёта, итог за встречу]
    ...
    def _note_drop(self, label: str, seconds: float, written: bool = True) -> None:
        ...
        if now - st[1] < self._DROP_REPORT_S:
            return
        if st[0] < 1.0:
            # Буфер дозревает до потолка кратно куску захвата, поэтому первое
            # переполнение — это четверть секунды. Ради неё не стоит ни строки
            # в ленте, ни тем более «звук не вернуть»: подождём, пока накопится
            # заметное (ревью 20.08, GLM).
            return
        st[1] = now
