...
class Thread:
    ...
    def add_answer(self, question: str, answer: str) -> bool:
        """Ответ — одной строкой «⚡ …», без строки вопроса.

        До этого ответы жили в своей панели и перекрывали нить; потом вопрос
        и ответ шли парой строк «? вопрос» + «⚡ ответ» — и нить забивалась
        вопросами (слова владельца 24.08: «вопросы убери, сам вопрос — оставь
        символ молнии и выделенный ответ»). Вопрос остаётся в аудите
        _hints.md («⚡ ответ на: …») — полотну он не нужен, ответ читается сам.
        """
