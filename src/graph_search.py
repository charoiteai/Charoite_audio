...
class GraphSearch:
    ...
    одним присваиванием под `_lock`, производной от ОСНОВЫ, которую обход
    снял под тем же замком первой строкой и передал явно: у публикации нет
    другого доступа к полю, так что «решение по одному снимку, запись
    другого» невыразимо, а не запрещено словами (три круга подряд ловили
    именно это в разных ветках `_walk` — Critical DS, круги 5–7 по №292).
    Обход запускается только из `refresh`, и сериализацию держит его
    `_scan_lock`: прямой вызов `_walk` был бы вторым писателем. Второй
    писатель — скажем, досыпка одной свежей заметки без полного обхода —
    сломает всё это молча: обход идёт секунды, и следующая публикация сотрёт
    его правку. Появится такой — он тоже идёт через `_publish` со своей
    основой, а `_scan_lock` берёт на себя.
    ...
    def _publish(self, base: Generation, gone: Iterable[str] = (), **changes: object) -> None:
        ...
        отдаёт сюда, читать поле заново публикация не умеет. `gone` — пути
        исчезнувших файлов: их векторы уходят тем же замком, что документы
        (DS M3 / GLM M10). Что это сторожит — `tests/test_graph_search.py`,
        тест на форму: присваиваний `_gen` в модуле ровно два."""
        with self._lock:
            for p in gone:
