...
class GraphSearch:
    ...
    связей, охват обхода). Пишет его ТОЛЬКО `_walk` (обходы сериализует
    `_scan_lock`), одним присваиванием под `_lock`, и всегда отталкиваясь от
    значения поля, прочитанного под тем же замком. Второй писатель — например
    досыпка одной свежей заметки без полного обхода — сломает это молча:
    обход идёт секунды, и следующая публикация сотрёт его правку. Появится
    такой — публикацию сводить в одну точку, а не дублировать присваивание; читатель берёт `gen =
    ...
    def _walk(self) -> None:
        ...
        with self._lock:
            ...
            if not fresh and not gone:
                ...
                # основа — то, что в поле ПРЯМО СЕЙЧАС, а не `current`, взятый до
                # обхода вне замка: иначе единственный выход-писатель умеет
                # откатить чужую публикацию на секунды обхода назад. Сегодня
                # второго писателя нет (`_scan_lock` сериализует обходы), но
                # это соглашение, и держать его должен код (Critical DS, круг 6)
                self._gen = dataclasses.replace(self._gen, skipped=scope[0], unread=scope[1])
                return
