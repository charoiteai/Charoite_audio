...
DEFAULT_MODEL = "qwen3.6:35b-a3b"       # если конфига нет: не хардкод по месту, а одно объявление
...
def _model() -> str:
    """Боевая модель из конфига владельца.
    ...
    """
    return _llm_cfg().get("model", DEFAULT_MODEL)
...
def _client() -> LLM:
    ...
    """
    base = dict(_cfg())
    base["llm"] = {"model": _model(), **_llm_cfg()}
...
def sufler_make_minutes() -> str:
    ...
    try:
        ...
        out = client.complete(
            ...
            model=_model(),  # из конфига: не тянем вторую тяжёлую модель поверх резидентной
