...
def _отпечаток_конфига(корень: pathlib.Path) -> tuple:
    """Что должно измениться, чтобы конфиг перечитался.

    Корень данных плюс время правки обоих кандидатов — пользовательского
    файла и примера из поставки. Одного корня мало: сервер живёт столько же,
    сколько сессия Claude Code, а владелец меняет модель в приложении прямо во
    время неё — с ключом по корню такая правка не долетала бы до перезапуска.
    """
    ...
    return (корень,
            когда(корень / "config" / "config.yaml"),
            когда(CODE_ROOT / "config" / "config.example.yaml"))
...
def _cfg() -> dict:
    ...
    Кэш отличается от снимка наличием канала отзыва — здесь это отпечаток
    (корень и время правки конфига). Без кэша один `sufler_make_minutes` читал
    ...
    global _cfg_кэш
    корень = _root()
    отпечаток = _отпечаток_конфига(корень)
