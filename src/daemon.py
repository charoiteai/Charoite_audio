...
def main():
    ...
    if owner_voice.collides_with_neutral(mic_label, other_label):
        # Правим ИСТОЧНИК метки, а не локальную переменную: её читает одна
        # ветка разметки из трёх, а в стенограмму метка течёт прямо из
        # захвата (pull_labeled отдаёт hub.SPEAKER["mic"]). Обнулив только
        # переменную, я оставил коллизионное имя во всех остальных путях —
        # и реплики владельца по-прежнему сливались с меткой, которую
        # name_loop переименовывает по имени из разговора (ревью 19.08,
        # шестой круг). «Я» — тот же дефолт, что при незаданном имени.
        hub.SPEAKER = {**hub.SPEAKER, "mic": "Я"}
        mic_label = ""
