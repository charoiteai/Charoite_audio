...
def main():
    ...
    def voice_label(channel_speaker: str, chunk) -> str:
        ...
        if spk_tracker is None:
            # Без трекера канальные метки честны в звонке — но и здесь имя,
            # совпавшее с нейтральной меткой, отдавать нельзя: name_loop
            # ищет «Собеседник N» и вправе переименовать такую подпись в имя
            # собеседника, уводя реплики владельца чужому (ревью 19.08).
            if channel_speaker == mic_label and owner_voice.collides_with_neutral(
                    mic_label, other_label):
                return other_label
    ...
    def _say_owner_state(signed: bool) -> None:
        ...
        if signed:
            ...
            note = ("собеседников в динамиках не слышно — похоже на очную "
                    ...
                    "нейтральными")
        elif owner_voice.collides_with_neutral(mic_label, other_label):
            # Та же проверка, что у решения: иначе имя вида «Собеседник 2»
            # отказывалось подписываться по коллизии, а человеку сообщалось
            # «в микрофоне слышно несколько человек» (ревью 19.08, четвёртый
            # круг).
            note = ("ваше имя в настройках совпадает с нейтральной меткой — "
                    "задайте другое (sufler.user_name), чтобы реплики "
                    "подписывались")
        else:
            ...
            for speaker, chunk in batch:
                ...
                for piece, n, raw_piece in jobs:
                    ...
                    if n is None:
                        ...
                        # Секунды такого куска в счётчики не идут: голос
                        # неизвестен, и приписать их некому — иначе доли
                        # считались бы по чужой речи (ревью 19.08).
                    else:
