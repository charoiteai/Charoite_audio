            ...
            if mem.status is not brain.Verdict.CONFIDENT:
                ...
                if added:
                    ...
                    append_hint(tr.path, f"[{dt.datetime.now():%H:%M}] ⏮ {title} (узлы)",
                                thread.full())
                    return
                # сводка по теме собрана по всему графу — при ней «почти ничего»
                # выбрасывало бы самое сильное свидетельство выдачи (GLM I2, №295)
                if (mem.status is not brain.Verdict.UNVERIFIED and not mem.dossiers) or mem.empty:
