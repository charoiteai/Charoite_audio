...
def main():
    ...
    def voice_label(channel_speaker: str, chunk) -> str:
        ...
        # живой разговор впятеро раньше срока (ревью 20.08, локальная голова).
    ...
    def stt_loop():
        while not stop.is_set():
            ...
            for speaker, chunk in batch:
                ...
                # выглядела бы одинокой, и правило `alone` резало бы её впятеро
                # раньше срока (ревью 20.08, DeepSeek I1). Чанк сюда попадает
                # уже после VAD, то есть это речь, а не тишина; лишний признак
                # «звонок» ошибается в безопасную сторону — оставляет прежние
                # пятнадцать минут.
                heard_by_channel.note(None, 0.0, is_mic=speaker == mic_label)
    ...
    def autostop_loop():
        ...
        while not stop.is_set():
            ...
            d = watch.tick(now=now,
                           ...
                           # ставится по чанку речи с системного канала после
                           # VAD — то есть по звуку, а не по факту, что канал
                           # существует. Ошибка возможна только в безопасную
                           # сторону: лишний «звонок» оставляет 15 минут.
