...
def main():
    ...
    # (сторож `test_каждая_точка_входа_закрывает_маску`).
    harden_umask()
    # Сразу за ней — назвать корень данных, до любого касания диска. Где они
    # лежат, знает тот, кто запускает: приложение ставит `CHAROITE_ROOT`
    # детям, launchd-агент несёт её в job, рецепт запуска — в README.
    #
    # Раньше здесь стоял `use_data_root(_root())`, и это была не называние, а
    # узаконивание догадки: `_root()` при пустом окружении возвращает третий
    # ответ канона (положение файла), тот ложится в `_given` и публикуется в
    # окружение — дети читают догадку как решение владельца, а позднейший
    # отказ в каноне до такого процесса уже не добирается (входной круг по
    # №332, DS C1). `--угадай-корень` — та же догадка, но названная вслух:
    # ручной прогон из checkout.
    try:
        require_data_root(__file__, guess_from_code="--угадай-корень" in sys.argv)
    except RootNotNamed as e:
        # Двумя каналами, и оба — существующие. `status` с `error: True`
        # приложение уже рисует красным (`SuflerService.consume`), поэтому
        # рецепт доходит до человека без единой правки Swift; `EXIT_ROOT_UNNAMED`
        # видит launchd и любой скрипт. Первая редакция слала `type: fatal` —
    ...
    # Статус хаба — в приложение одной формой: липкость и окраску ставит
    # писатель (`stt_runtime.Status`), сериализует контрактный модуль; демон
    # больше не гадает по подстрокам маркеров (№310, круг по №235)
    hub.on_status = lambda s: emit(stt_runtime.status_event(s))
    ...
    try:
        ...
        # До 13.09 падение старта уходило трейсбеком в stderr мимо finally: ни
        # строки в UI, ни уборки .pcm-заготовок (кадров в них нет). Скажем вслух
        # и закроем то, что успели открыть (аудит 13.09, DS M5 / GLM M5).
        emit_error(f"захват не стартовал: {short_error(e)}")
        try:
            hub.stop()
    ...
    def voice_label(channel_speaker: str, chunk) -> str:
        ...
        """
        # Признак «собеседников слышно» ставим ПЕРВЫМ делом и независимо от
        # трекера: он канальный, а трекер в поставку не входит и может не
        # определить голос. Без этого запись обычной удалённой встречи
        # выглядела бы как разговор с самим собой, и правило `alone` резало бы
    ...
    def _owner_label(voice: int | None, speaker: str, neutral: str) -> str:
        ...
        if voice is None:
            ...
            # не довод ни за, ни против подписи. Сбрасывать здесь состояние
            # значило рушить гистерезис на каждом тихом куске и стрелять
        ...
        while not stop.is_set():
            ...
            if stt_runtime.lag_transition(lagging, next_lagging):
                ...
                if lagging:
                    ...
                    # Порог — chunk_s, не 30: кадры пишет _pump из своего
                    # потока непрерывно, у живого входа возраст доли секунды
                    # независимо от скорости STT. Порог 30с пропускал ложный
            ...
            for speaker, chunk in batch:
                ...
                # раскладки) до него не доходили: запись живого звонка
                # выглядела бы одинокой, и правило `alone` резало бы её втрое
                # раньше срока (ревью 20.08, DeepSeek I1). Чанк приходит после
                # энергетического гейта — RMS-порог в дБФС, а не модель VAD и
                # тем более не распознанная речь (audio.py, `is_speech`). Отсюда
                # обе стороны ошибки, и вторая не безопасна: системный «дзынь»
                # или музыка поставят признак зря (останется 15 минут — не
                ...
                for job_index, (piece, n, raw_piece) in enumerate(jobs):
                    ...
                    if n is None:
                        ...
                        # Раскладка не решила — метка канальная, как было до
                        # фичи. Пропускать её через политику бессмысленно:
                        # голос неизвестен, и «нейтралью» пришлось бы
                        # подставлять сам канал (ревью 19.08, круги 2 и 5).
                        # Секунды такого куска в счётчики не идут: приписать
                        # их некому.
                        name = speaker
                ...
                for (name, text, _head), (head, seam_with) in zip(rows, seams):
                    ...
                    last_label_by_channel[speaker] = stt_runtime.next_channel_label(
                        prev_label, added_labels)
    ...
    def expand_topic(title: str = ""):
        ...
        Живой контекст подтягивает память в системный промпт молча. Здесь
        ...
        try:
            ...
            try:
                ...
                if added:
                    ...
                    emit({"type": "status", "text": f"⏮ память недоступна ({why})"})
                return
            ...
            if mem.status is not brain.Verdict.CONFIDENT:
                ...
                if added:
                    ...
                    append_hint(tr.path, f"[{dt.datetime.now():%H:%M}] ⏮ {title} (узлы)",
                                thread.full())
                    return
                if brain.absence_only(mem):   # политика — в таблице фасада (GLM M1, №295)
        ...
        while not stop.is_set():
            ...
            try:
                ...
                if not brief_done and len(full) >= 600:
                    ...
                    if picks:
                        brief_done = True
                        lines = ["⏮ Контекст к встрече из памяти прошлых встреч:"]
                        for _s, p in picks:
        ...
        while not stop.is_set():
            ...
            try:
                ...
                if out.strip():
                    ...
                    # normalize стоял только на РУЧНОМ «Протоколе», а
                    # автоматической финализации после стопа нет — черновик и
                    # есть итоговые минутки встречи, и его проза была невидима
                    # окну задач (жалоба 31.08: «58 задач как месяц назад»).
    ...
    def live_context_loop():
        ...
        while not stop.is_set():
            ...
            try:
                ...
                mem = None
            # узлы графа и фрагменты памяти — одним блоком фасада: бюджет делится
            # по долям (узлы первыми под общий кап съедали фрагменты — DS I1 r4), шапка
    ...
    def autostop_loop():
        ...
        while not stop.is_set():
            ...
            d = watch.tick(now=now,
                           ...
                           # Собеседников слышно не было ни разу. Признак
                           # канальный (не диаризация), но не абсолютный: он
                           # ставится по чанку с системного канала после VAD —
...
if __name__ == "__main__":
    # Код возврата уезжает наружу: `main()` голым вызовом выбрасывал его, и
    # процесс завершался нулём даже отказавшись стартовать — launchd с
    # KeepAlive и любой `&&`-скрипт видели успех (круг 1 по коду №332, обе
    # головы независимо)
    sys.exit(main())
