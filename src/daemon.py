...
def main():
    ...
    try:
        ...
        # причина уезжает через границу процесса значением, а не подстрокой:
        # приложение перезапускает демона трижды, и «корень не назван» — не
        # тот случай, когда повтор поможет (входной круг, DS I5/GLM 5)
        emit({"type": "fatal", "reason": "root_unnamed", "text": str(e)})
