...
def main():
    ...
    # Где лежат данные, знает точка входа, а не графовый слой: `__file__`
    # библиотеки отвечает на другой вопрос — где лежит КОД. Пока граф живёт
    # в `src/` рядом с данными, разницы нет; из отдельно поставленного пакета
    # вывод из положения дал бы site-packages, то есть дефолты вместо
    # настроек человека (№327).
    graphs.use_data_root(ROOT)
