...
#: Корень данных, заданный тем, кто этот код использует. Пока не задан —
#: выводится из переменной окружения или положения модуля, как раньше.
#: Модульная переменная, а не аргумент каждого вызова: корень один на процесс
#: и меняться посреди работы не должен — двадцать четыре места читают путь
#: графа, и разойдись они, граф писался бы в одно место, а искался в другом.
_root: pathlib.Path | None = None


def use_data_root(path) -> pathlib.Path:
    """Сказать графу, где лежат данные человека.

    Пока граф жил внутри репозитория, корень можно было вывести из положения
    файла: код и данные лежали рядом. Отдельно поставленный пакет так не может
    — рядом с ним `site-packages`, а не чужие встречи и конфиг. Выведенный
    оттуда путь не ошибка в вычислении, а ответ не на тот вопрос: где лежит
    код, вместо того где лежат данные.

    Поэтому корень приходит снаружи. Задаёт его тот, кто знает: приложение из
    своих настроек, скрипт из окружения, тест из временной папки.
    """
    global _root
...
def data_root() -> pathlib.Path:
    """Где лежат данные: заданный корень, переменная окружения или положение файла.

    Порядок неслучаен. Заданный явно — сильнее всего: он единственный не
    догадка. `CHAROITE_ROOT` — как было, и читает его канон (`resolve_root`),
    а не своя копия правила: копия без `strip()` делала `CHAROITE_ROOT=" "`
    относительным корнем, и относительный `graph_dir` снова зависел бы от cwd
    (круг-1 по PR #385, Sonnet). Вывод из положения модуля остаётся последним
    и верен, только пока модуль лежит в дереве репозитория; из установленного
    пакета он дал бы `site-packages`, и весь конвейер молча читал бы дефолты
    вместо настроек человека.
    """
