"""Где лежат графы и дверь окружения приложения к слою графа — для дневного и ночного контура.

`open_search` и `revise_cores` собирают то, чего модули графа не знают сами: каталог кэша
векторов и ночное окно живой встречи (№365).
...
def search_cache_dir() -> pathlib.Path:
    """Каталог кэша векторов поиска по графу — одно место на всех, кто строит индекс.

    Индекс поиска корня данных не знает: каталог кэша ему передают
    (`GraphSearch(data_dir=…)`). Писатели векторов (разбор встречи, ночь, бенч) и
    читатель (демон) обязаны собрать один и тот же путь, иначе демон не увидит ни
    одного вектора, а ночь напишет «всё собрано» (Opus I5 входного круга №365)."""
