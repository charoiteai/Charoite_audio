...
def use_data_root(path, *, replace: bool = False) -> pathlib.Path:
    ...
    `replace=True` — осознанная смена: снятие старого и запись нового идут
    под одним замком, поэтому состояния «корня нет вовсе» не возникает ни на
    миг. Без этого промежуток между «забыть» и «назвать» отвечал бы корнем
    КОДА — в бандловой установке это подписанный read-only `.app`, куда
    ребёнок, запущенный в окне, попробовал бы писать (круг 3 по коду №327,
    DS I1).
    """
    ...
    with _lock:
        ...
        if действующий is not None and действующий != root:
            raise RuntimeError(
                ...
                f"сменить его осознанно — use_data_root(..., replace=True)")
        _given = root
