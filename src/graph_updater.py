...
def graph_stamp(stamp: str, bare: str, tdir: pathlib.Path) -> str:
    """Имя, под которым встреча живёт в графе: минута или своя секунда.

    Демон после краха поднимается за две секунды — обе встречи попадают в
    одну минуту, а весь конвейер строился от МИНУТНОГО штампа. Вторая
    затирала заметку первой (`Встречи/{stamp}.md`), уводила её строку в MOC,
    забирала под свою тему папку архива и файлы разбора, а метку отправки
    фактов (`logs/brain_sent/{stamp}.txt`) видела как свою — и её собственные
    факты не доезжали ни до узлов графа, ни до памяти (ревью 20.08, GLM).

    Минутное имя достаётся САМОЙ РАННЕЙ встрече минуты, остальные живут под
    посекундным. Порядок берём из имён стенограмм, а не из очереди прогонов:
    иначе тот же файл назывался бы по-разному при пересборке и повторе.
    Легаси-встреча без секунд сравнивать не с чем — за ней минута остаётся.
    """
    if bare == stamp:
        return stamp
    minute = {bare}
    try:
        for f in tdir.glob(f"{stamp}*.md"):
            if stamp_of(f.stem) is not None:   # None у производных (_minutes, _разбор)
                minute.add(parse_stem(f.stem)[1])
    except OSError:
        return stamp                            # папку не прочитать — прежнее поведение
    # Переименованная встреча носит минутный штамп, и он же лексикографически
    # меньше любого посекундного: префикс короче — значит первая по времени.
    return stamp if min(minute) == bare else bare
...
def main():
    ...
    # Дальше `stamp` — имя встречи В ГРАФЕ и в производных файлах. Оно
    # совпадает с минутным, пока минута принадлежит одной встрече.
    stamp = graph_stamp(stamp, bare, tpath.parent)
