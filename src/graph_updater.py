...
# Коды возврата живут в exit_codes (без импортов): их читают и пересборка, и
# импорт, которому верхний уровень этого модуля не по карману (№173).
from exit_codes import EXIT_NO_GRAPH, EXIT_NO_SPEECH  # noqa: E402,F401
...
def canon_link(graph: pathlib.Path, name: str, default_folder: str | None = None) -> str:
    ...
    if default_folder == "Ядра" and p is not None and p.parent.name != "Ядра":
        # Тема заведена и узлом другого типа (Системы/X), и ядром — upsert_core
        # держит параллельное Ядра/X. Ссылка «## Ядра» в заметке встречи должна
...
def _one_edit_away(a: str, b: str) -> bool:
    """Расстояние Левенштейна ≤ 1: одна замена, вставка или удаление символа."""
    if a == b:
        return True
...
def entity_node_verdict(graph: pathlib.Path, folder: str, name: str) -> tuple[str, list[str]]:
    ...
    «Kwen 32B» при узле «Qwen 32B» — вторым узлом навсегда, а кандидаты
    find_canonical при неоднозначности («не гадаем») уходили в никуда (DS F4
    и критика GLM r1b, аудит памяти 07.09; №193). Теперь мусор — не узел;
    ...
    hits: list[str] = []
    for f in _alias_index([f for f in files if f.parent.name != "Люди"]).get(key, []):
...
_HELD_LINE_RE = re.compile(
    r"^- (\d{4}-\d{2}-\d{2}) \[\[([^\]]+)\]\] · «(.+?)» \((.+?)\) — (.+?): "
    r"(\[\[[^\]]+\]\](?:, \[\[[^\]]+\]\])*)(?: — .*)?$")


def _held_pairs(text: str) -> list[tuple[str, str, str, tuple[str, ...]]]:
    """Строки `_Кандидаты.md` → (ключ имени, папка, почему, кандидаты). Разбираем
    только собственный формат строки; хвосты «— повтор ×N» и пометки ревизии
    после « — » отрезаются. Ключ — name_key, а не сырой текст модели: «Kwen 32B»
    и «kwen 32b» — одна пара (круг 1, DS I1); повтор считается по строкам, а не
    подстрокой — строка с бо́льшим набором кандидатов не содержит меньшую
    (DS M2 / GLM I3)."""
    out = []
    for ln in text.splitlines():
        ...
        if not m:
            continue
        _day, _mtg, nm, typ, why, links = m.groups()
        folder = "Ядра" if typ == "ядро" else ENT_FOLDER.get(typ, "Системы")
        out.append((name_key(nm), folder, why, tuple(re.findall(r"\[\[([^\]|]+)", links))))
    return out


def _safe_to_auto_alias(name: str, cand: str) -> bool:
    """Опечатка в одну букву — достаточное основание для псевдонима без человека
    только при общем числе или общем слове от четырёх букв: «Kwen 32B» → «Qwen
    32B», «Реестр Витрен» → «Реестр Витрин». Однословное без цифр («Препрод» →
    «Препрот») — два разных объекта не реже, чем опечатка; там только счётчик
    (круг 1, DS I2 / GLM К1)."""
    stem = cand.split("/", 1)[-1]
    a, b = name_key(name), name_key(stem)
    if re.findall(r"\d+", a) and re.findall(r"\d+", a) == re.findall(r"\d+", b):
        return True
    wa = {name_key(w) for w in safe_name(name).split() if len(name_key(w)) >= 4}
    wb = {name_key(w) for w in safe_name(stem).split() if len(name_key(w)) >= 4}
    return bool(wa & wb)


def _journal_held_entity(graph: pathlib.Path, folder: str, name: str, typ: str,
                         verdict: str, cands: list[str], meeting_link: str) -> pathlib.Path | None:
    ...
    HELD_REPEAT_AUTO_ALIAS-го раза — псевдоним в узле-кандидате. Порядок:
    решение → действие → запись о нём (DS M3). -> путь узла, куда записан
    псевдоним, иначе None."""
    ...
    same = [c for (k, f, w, c) in _held_pairs(text)
            if k == name_key(name) and f == folder and w == _HELD_WHY[verdict]]
    repeat = len(same) + 1
    line = f"- {day} {mark} — {_HELD_WHY[verdict]}: {links}"
    aliased = None
    if repeat >= HELD_REPEAT_AUTO_ALIAS:
        ...
        if (verdict == "near" and len(cands) == 1 and all(c == tuple(cands) for c in same)
...
def _auto_alias(graph: pathlib.Path, cand: str, name: str, meeting_link: str) -> pathlib.Path | None:
    """Псевдоним `name` в шапку узла-кандидата «Папка/Имя» по повтору (№236) —
    через гейт потери обновления, как все правки узлов. Заглушку-редиректа не
    ...
    (0 изменений — псевдоним уже был, это тоже «есть»). Флага успеха до записи
    нет: он рапортовал успех при проигранной гонке (круг 1, DS C2 / GLM C2).
    -> путь узла или None."""
    ...
    try:
        safe_write.rewrite_file(path, transform, "псевдоним по повтору")
    except (OSError, safe_write.LostRace) as exc:
        state["why"] = str(exc)
    if "why" in state:
        _journal_graph_event("псевдоним по повтору не записан", f"{cand} ← «{name}»: {state['why']}", meeting_link)
        return None
    _journal_graph_event("псевдоним по повтору", f"{cand} ← «{name}»", meeting_link)
    return path
...
def entity_line(graph: pathlib.Path, e: dict, held: dict[tuple[str, str], tuple[str, list[str]]]) -> str:
    ...
    if verdict == "aliased":
        # псевдоним по повтору (№236): встреча легла в узел-кандидат — ссылка на
        # него с именем из разбора; canon_link здесь не годится, узел может
        # лежать в другой папке, чем сказал тип сущности
        return f"- [[{cands[0]}|{safe_name(name)}]] {head} _({_HELD_WHY[verdict]})_"
...
def apply_entities(graph: pathlib.Path, ents: list[dict], meeting_link: str) -> dict[tuple[str, str], tuple[str, list[str]]]:
    ...
    for e in ents:
        ...
        if verdict == "existing" and cands:
            # узел найден по псевдониму в ДРУГОЙ папке, чем сказал тип (тип — самое
            # ненадёжное поле разбора): пишем в него по его адресу, ссылка в заметке —
            # на него же; в папке типа фантом не заводим (круг 1, DS C1 / GLM C1)
            tf, tstem = cands[0].split("/", 1)
            upsert_entity(graph, tf, tstem, typ, desc, meeting_link, "")
            held[(folder, name)] = ("aliased", cands)
            continue
        ...
        if aliased is not None:
            ...
            # СЕЙЧАС, со следом в его «## Встречи»: через месяц автосклейка должна
            # быть отличима от псевдонима, поставленного человеком (DS К2 / GLM К1)
            upsert_entity(graph, aliased.parent.name, aliased.stem, typ, desc, meeting_link,
                          f"псевдоним «{name}» записан по повтору")
            held[(folder, name)] = ("aliased", [f"{aliased.parent.name}/{aliased.stem}"])
            continue
        held[(folder, name)] = (verdict, cands)
    landed = {n: c[0] for (_, n), (v, c) in held.items() if v == "aliased"}
    if landed:
        print("граф: по псевдониму легло в узел — " + "; ".join(f"{n} → {t}" for n, t in landed.items()))
    rest = {n: (v, c) for (_, n), (v, c) in held.items() if v != "aliased"}
    if rest:
        print("граф: узла не получили — " + "; ".join(
            f"{n} ({_HELD_WHY[v]}{': ' + ', '.join(c) if c else ''})" for n, (v, c) in rest.items()))
    return held
...
def _read_node(p: pathlib.Path, meeting_link: str = "") -> str | None:
    ...
    и каждый ретрай падал на том же узле (аудит 13.09, DS I3 / GLM I3).
    Событие — в журнал, узел пропускается."""
    try:
...
def _current_description(text: str) -> tuple[str, int, int]:
    """(описание, начало, конец) — тело узла сразу после «# Имя» до служебной
    строки («_(последнее упоминание…»), горизонтальной черты или заголовка
    раздела. Пустые строки внутри — часть описания: иначе второй абзац старого
    описания оставался сиротой под новым (DS/GLM r1 по #539). Заголовок
    ...
    for ln in text[start:].split("\n"):
        s = ln.strip()
        if s.startswith(("## ", "_(последнее упоминание:", "---")) or ln.startswith("# "):
            break
        end += len(ln) + 1
...
def resolve_core_path(d: pathlib.Path, name: str,
    ...
    while True:
        ...
        try:
            ...
            return p            # нечитаемый — не заглушка; upsert_core скажет и пропустит (DS I3 / GLM I3)
        if not redirects.is_merged(text):
            return p
...
def upsert_core(graph: pathlib.Path, core: dict, meeting_link: str, stamp: str,
    ...
    d = graph / "Ядра"
    d.mkdir(parents=True, exist_ok=True)
    if not (d / f"{safe_name(core['имя'])}.md").exists():
        # Теме отвечают ДВА ядра и больше («Пилот проект 2026» и «… 2027» на
        # «Пилот проект»): find_canonical по правилу «не гадать» отдаёт None, и
        # рядом вырастало третье ядро на ту же тему. Дубль хуже пропуска:
    ...
    if p.exists():
        ...
        if not seen_link:
            text = _insert_chronicle_line(text, stamp_line + superseded)
        elif changed:
            ...
            if not found:   # ссылка есть, но не в хронике (человек перенёс) — след всё равно нужен
                ...
                text = _insert_chronicle_line(text, line)
...
def append_moc_line(moc: pathlib.Path, meeting_link: str, line: str) -> bool:
    """Строка встречи в _MOC.md, если её там ещё нет. Проверка — по границе
...
def rebuild_folder_index(graph: pathlib.Path, folder: str) -> None:
    ...
    for p in sorted(d.glob("*.md")):
        ...
        # заголовок — строкой (CRLF, конец файла без перевода строки), без секции —
        # ноль, а не весь узел: фолбэк снова считал ссылки из прозы (DS M7 / GLM M5)
        m = re.search(r"^## Встречи[ \t]*\r?$\n?(.*?)(?=^## |\Z)", text, re.M | re.S)
        stamps = set(re.findall(r"\[\[Встречи/(\d{4}-\d{2}-\d{2}[^\]|#]*)", m.group(1) if m else ""))
...
def main():
    ...
    touched |= {c[0].split("/", 1)[0] for (v, c) in held.values() if v == "aliased" and c}
    ...
    if people or anon:
        md += (["## Участники"]
               ...
               + [""])
    if ents:
        md += ["## Сущности"] + [entity_line(graph, e, held) for e in ents] + [""]
    if cores:
    ...
    if graph_ok and moc.exists():
        append_moc_line(moc, meeting_link,
                        f"- [[{meeting_link}|{title or stamp}]] — {', '.join(topics[:2]) if topics else 'встреча'}")
...
def cloud_enrich_prompt(*, transcript_name: str, folder: pathlib.Path,
    ...
    analysis = (
        ...
        "расшифровкой). Поручения, которых нет в минутках, вынеси отдельно, "
        "в самый конец ревизии, под заголовком СТРОГО «## Восстановленные "
        ...
        "стенограммы, имя — участник встречи. По этому разделу Чароит переименует "
        "заголовки реплик и участников минуток; поручения минуток под ошибочной "
        "меткой сними в «## Снятые поручения» и верни с верным именем в "
    ...
    return head + analysis + (
        ...
        "Исправил метку стенограммы — в «## Участники» заметки этой встречи "
        "назови верного человека, а из узла Люди с ошибочным именем убери строку "
        "этой встречи в «## Встречи» и её связи (перенеси их в узел верного "
        "человека), если этого человека на встрече не было.\n"
