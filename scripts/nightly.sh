...
# Два правила, у каждого своё разрешение в конфиге (по умолчанию — только
# отчёт): конфликтные копии «Имя N» документов встреч уезжают в резерв вне
# iCloud (sufler.dedup_copies), побайтовые копии от 4 КБ связываются
# жёсткими ссылками (sufler.dedup_files). Шаг идёт сразу после доктора: он
# стоит секунды (12,7 с на рабочем графе 23.09), а в конце ночи его срезал
# потолок — 12 ночей подряд с 10.09 (№361). Не путать с ревизией ядер ниже —
# та сшивает СМЫСЛОВЫЕ дубли тем.
$PY scripts/dedup_graph.py --all-graphs || { echo "⚠️ дедупликация файлов не отработала"; FAILED="$FAILED дедуп"; }
...
if overdue; then
  ...
  $PY scripts/tier3_cores.py --all-graphs --auto $TIER3_MODE || {
    case $? in
      ...
      # 4 — потолок ночи оборвал ревизию: досмотрено не всё. Недосмотренные
      # ядра и досуженные пары лежат рядом с отметкой, завтра ревизия
      # продолжит с места обрыва. Не авария, но и не «ok».
...
if overdue; then
  ...
  # в FAILED, и статус ночи — failed (write_status), но rc не трогаем: шаг
  # необязательный, а обновление CLI самовосстанавливается — красный код
  # launchd за него обесценил бы сигнал (критика GLM/DS по #499);
  # код 2 — часть тем не проверена (сбой шага, аудит 05.09)
  $PY scripts/nightly_dossier_review.py --all-graphs || {
    case $? in
      3) echo "❌ ревизия досье: CLI облака не отвечает"; FAILED="$FAILED ревизия-досье(cli)" ;;
...
if overdue; then
  ...
  $PY scripts/nightly_claude_cores.py || {
    case $? in
      3) echo "❌ облачная ревизия ядер: CLI облака не отвечает"; FAILED="$FAILED claude-cores(cli)" ;;
