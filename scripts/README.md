...
- `merge_graphs.py` — merge a split-off graph back into the main one: new files move, name collisions get appended as a "moved from" section (donor frontmatter stripped), meeting lines migrate into the receiver's `_MOC.md`, the donor `_MOC.md` becomes a "merged into" note. Dry-run by default; `--apply` executes.
