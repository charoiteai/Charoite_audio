...
- `fix_action_items.py` — a one-off normalization of action-item formatting in minutes written before the daemon started normalizing them; only the format changes, and a status already set (done `[x]`, cancelled `[-]`, reopened by hand) stays as it is. Dry-run by default; `--apply` needs `CHAROITE_ROOT` and writes under the shared graph lock.
