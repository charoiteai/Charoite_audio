...
_PATH = re.compile(rf"(?<![A-Za-z0-9_.-])(?:{DIST_DIR}/[A-Za-z_][A-Za-z0-9_]*"
                   r"(?:/[A-Za-z_][A-Za-z0-9_]*)*/[A-Za-z_][A-Za-z0-9_]*\.(?:py|sh)"
                   rf"|(?:{FLAT_DIR}|scripts|app)/[A-Za-z_][A-Za-z0-9_]*\.(?:py|sh)"
...
def _owner(rel: str) -> str:
    """Кто предоставляет файл: дистрибутив или плоский корень."""
...
def inventory(repo: pathlib.Path | None = None) -> Inventory:
    ...
    # имена берёт ФОРМА, а не `module_of`: схлопнутся ли два файла в один узел —
    # свойство раскладки, и правило `KINDS`, отправившее файл вне области, его
    # не отменяет (круг 4 по коду №328, GLM M3)
    problems += packaging_conflicts({rel: m for rel in files if (m := form(rel).module) is not None})
