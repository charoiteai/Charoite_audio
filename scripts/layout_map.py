...
def _bootstrap_budget(rel: str) -> int:
    """Сколько подъёмов у вставки пути действительно есть в рецепте.

    Рецепта два, и оба заканчиваются каталогом `src/`: у модуля `src/` цель —
    его собственный каталог, один подъём (`Path(__file__).parent`); у скрипта
    — `src/` рядом с корнем репозитория, подъём ДО КОРНЯ
    (`Path(__file__).parent.parent / "src"`, два шага). Глубина файла как
    бюджет (`rel.count("/") + 1`) для скрипта совпадала с рецептом, а модулю
    `src/` прощала лишний шаг — подъём выше корня репозитория, которого в
    рецепте нет. Константа 1 была хуже обоих: правило ловило собственный
    рецепт — 26 строк формы «file» на скриптах, переведённых по нему же
    (№338, замер после перевода 25 входов).
    """
    if rel.startswith(f"{FLAT_DIR}/"):
        return 1
    return rel.count("/") + 1
