...
## 3. Системный звук (звонки)

Без него Чароит слышит только микрофон (очные встречи работают сразу).
Для звонков (Zoom/Meet/любые) есть два пути.

**Ничего не делать (macOS 14.4 и новее).** Приложение само поднимает захват
системного звука на время встречи — через Core Audio process tap, средствами
системы. При первой записи macOS спросит разрешение «Запись экрана и
системного звука»; звук продолжает идти в колонки как обычно, устройство
исчезает после встречи. Ставить драйверы и переключать выход не нужно.

**Запасной путь — BlackHole** (старая macOS, отказ в разрешении):
...
⚠️ Если стоит и то, и другое: держать выход системы на обычном устройстве, а
не на Multi-Output. Тап на многовыходном устройстве отдаёт тишину, а на честных
двухканальных — полный уровень.

Раздельные каналы дают бесплатную диаризацию «вы/собеседники» и фильтр эха.

**Если в статусе появилось «обычная конфигурация не принята».** Устройство тапа
живёт на частоте физического выхода (44.1 или 48 кГц), а конвейер просит 16 кГц —
не всякий выход принимает такой запрос и отвечает `-10851`. Демон в этом случае
не сдаётся, а перебирает конфигурации и пишет, какая подошла. Делать ничего не
нужно: звук идёт, качество распознавания не меняется. Строчка нужна для
диагностики — по ней видно, что именно устройству не понравилось.
