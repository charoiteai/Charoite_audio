...
## 3. Системный звук (звонки) — ничего делать не нужно
...
системного звука» — нажмите «Разрешить». Всё: драйверы, Audio MIDI Setup и
переключение выхода больше не нужны, звук продолжает идти в колонки как
обычно, а на macOS 15 и новее тем же потоком приходит и микрофон.
