...
1. Поставьте Ollama, остальное сделает мастер первого запуска: спросит имя и
   папку графа, предложит набор моделей под память этой машины и поставит
   его. Python-контур и код демона едут внутри бандла — ни `git clone`, ни
   venv, ни `pip` не нужны.
2. Настройки (⌘,), если нужно что-то поменять:
   - **Папка Charoite_audio** — только для запуска из исходников: укажите
     склонированный репозиторий, и приложение возьмёт код оттуда;
   - **Ollama** — адрес сервера (по умолчанию `http://localhost:11434`);
   - кнопка «Проверить» покажет, что демон, Ollama и граф на месте.
3. Путь графа лежит в `config/config.yaml` (`sufler.graph_dir`) внутри вашей
   рабочей папки — мастер его пишет, демон читает.
