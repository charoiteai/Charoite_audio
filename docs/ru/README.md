...
Приложение подписано Developer ID, но не нотаризовано, поэтому при первом
запуске macOS его блокирует: Системные настройки → Конфиденциальность и
безопасность → *Всё равно открыть* (либо
...
дневника (безопасно в фоне, с таймером Live Activity в Dynamic Island),
