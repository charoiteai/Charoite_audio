...
|----|----|----|----|----|
| **8 ГБ** | `qwen3.5:4b` (3,2 ГБ) | та же | да | Одна модель на обе роли; дежавю и ревизия ядер выключены — оба поднимают `bge-m3`, который рядом не помещается |
| **16 ГБ** | `qwen3.5:4b` (3,2 ГБ) | та же | да | Та же модель, что на 8 ГБ, плюс семантическая память: дежавю и ревизия ядер (`bge-m3` помещается) |
| **32 ГБ** | `qwen3.8:27b-mlx` (16,9 ГБ) | `qwen3.5:4b` | да | Точнее по цитатам (96%), разбор втрое дольше — он фоновый |
| **64 ГБ+** | `qwen3.6:35b-mlx` (20,4 ГБ) | `qwen3.5:4b` | да | Рабочий набор: 42 решения и 39 ядер на встречу, медиана 57 с |

Цифры — бенч 19.08 на трёх реальных встречах (`scripts/bench_extract.py`,
методика 14.08). Он же снял старое правило «ниже 16 ГБ граф выключен»:
`qwen3.5:4b` разобрала все три встречи (31 решение, 30 ядер, 96% цитат) —
находок больше, чем у `gemma4:12b`, и втрое быстрее. Ломается на JSON не
класс размера, а конкретные модели: `GigaChat3.1-10B` не дала разбора в
пяти частях из шести и в таблицу не попала. На 4 ГБ — только STT; `llm.base_url` на другую свою машину — только с явным `llm.allow_remote: true` (стенограммы уходят с этого компьютера; под `CHAROITE_NO_CLOUD` запрещено — см. `docs/MODELS.md`).
...
бандл подписан ad-hoc — или подписан, но не нотаризован, — и при первом
запуске macOS его блокирует: Системные
...
CHAROITE_ROOT="$PWD" .venv/bin/python src/main.py     # живая стенограмма + подсказки в терминале
```
