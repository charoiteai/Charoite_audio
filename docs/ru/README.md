...
Приложение пока подписано ad-hoc (Developer ID и нотаризация — в планах),
поэтому при первом запуске macOS его блокирует: Системные настройки → Конфиденциальность и
