...
При первом запуске приложение до предложения записи проверяет весь реальный
контур встречи: `.venv`, демон и конфиг, Python-зависимости, микрофон и
аудиовходы, модели Ollama и папку графа. Отсутствие BlackHole, `bge-m3` или
опционального графа показано как ограничение, а не как неотличимая «поломка».
