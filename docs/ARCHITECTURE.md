...
never counted by hand), and the declared entry points: every executable file
(`src/*.py` with a `__main__`, `scripts/*.py`, `scripts/*.sh`) is either named
by code — Swift, shell, CI workflows, pre-commit hooks, python subprocesses —
or declared manual. Mentions are read from code, not prose: python string
literals via the AST without docstrings, Swift and shell without comments. The gate is `tests/test_import_boundaries.py`,
