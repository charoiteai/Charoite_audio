...
launches whom by path (the Swift app calls `src/daemon.py` in 22 places,
...
arrows, an allowlist of the edges that still point upward (13 on 19.09, each
with the card that removes it, regenerated by `scripts/layout_map.py --regen`,
never counted by hand), and the declared entry points (33 paths that Swift,
shell and python subprocesses call). The gate is `tests/test_import_boundaries.py`,
...
point nobody calls fails too. Lazy imports inside functions count. The map
`docs/design/layout.md` is generated from the same file; it is not edited by
hand. Not import-linter: it needs an importable package, and a flat folder of
