...
Последним шагом она снимает с релиза флаг pre-release и объявляет его
latest (`gh release edit --prerelease=false --latest`).

Это и есть гейт: release-please создаёт каждый выпуск как **pre-release**
(`"prerelease": true` в `.github/release-please-config.json`), а апдейтер
спрашивает `/releases/latest`, куда GitHub pre-release не отдаёт. Выпуск без
подписи для приложения не существует — вместо отказа «у выпуска нет верной
подписи манифеста» у каждого пользователя. Сборка и нотаризация на флаг не
смотрят: `release: published` приходит и для pre-release, `gh release list`
его тоже видит. Пока владелец не подписал — пользователи остаются на
предыдущем выпуске; упала загрузка подписи — релиз остаётся pre-release.
Ручной `gh release edit --latest` в обход скрипта — ровно то, от чего гейт
защищает: не делать.

Граница гейта: release-please помечает выпуск pre-release по формуле
`prerelease && (версия с суффиксом || major == 0)` — на 0.x флаг работает
сам, с 1.0.0 перестанет. Тест-растяжка
`test_release_gate_tripwire_pre_major_versions_only` упадёт на первой версии
≥ 1.0: тогда гейт переносится (например, pre-release ставит сам release-app
сразу после сборки, приняв окно в несколько минут), а не теряется молча.
