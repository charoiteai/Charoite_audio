...
Она скачивает `Charoite.app.zip.sha256`, подписывает его сырые байты
(raw ed25519 → base64) ключом `~/.config/charoite/update_manifest_ed25519.pem`
и прикладывает `Charoite.app.zip.sha256.sig` к релизу. Без этого шага
пользователи увидят «у выпуска нет верной подписи манифеста».
