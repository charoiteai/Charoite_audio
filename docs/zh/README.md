...
|----|----|----|----|----|
| **8 GB** | `qwen3.5:4b`（3.2 GB） | 同左 | 有 | 一个模型兼顾两种角色；既视感与核心复核关闭——两者都会拉起 `bge-m3`，而它放不下 |
| **16 GB** | `qwen3.5:4b`（3.2 GB） | 同左 | 有 | 与 8 GB 相同的模型，另加语义记忆：既视感与核心复核（`bge-m3` 放得下） |
| **32 GB** | `qwen3.8:27b-mlx`（16.9 GB） | `qwen3.5:4b` | 有 | 引用更准确（96%）；解析慢三倍，但它在后台运行 |
| **64 GB+** | `qwen3.6:35b-mlx`（20.4 GB） | `qwen3.5:4b` | 有 | 日常配置：每场会议 42 条决定、39 个核心，中位数 57 秒 |

以上数字来自 8 月 19 日在三场真实会议上的基准测试（`scripts/bench_extract.py`，
沿用 8 月 14 日的方法）。它同时废除了「低于 16 GB 关闭图谱」的旧规则：
`qwen3.5:4b` 解析了全部三场会议（31 条决定、30 个核心、96% 引用），发现比
`gemma4:12b` 更多，且快三倍。破坏 JSON 模式的不是参数量级，而是具体模型：
`GigaChat3.1-10B` 有六分之五的分片没有给出解析，因此未列入表格。
...
模型运行时与模型由应用**自行安装**：首次启动时它会显示缺少什么，并给出一个
按钮——无需终端。
