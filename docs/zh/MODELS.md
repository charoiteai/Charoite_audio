...
| **8 GB** | `qwen3.5:4b` (3.2 GB) | 同一模型 | GigaAM | 逐字稿、要点、会议纪要草稿、基础提示。一个模型兼任两个角色;无并行 Claude 层。图谱可用（见下方 8 月 19 日基准），但既视感关闭：`bge-m3` 会在系统旁再占 1.2 GB。 |
| **16 GB** | `gemma4:12b` (7.0 GB) | `qwen3.5:4b` (3.2 GB) | GigaAM | 完整实时循环:提示 + 要点 + 会议纪要并行。抽取中的引用全部可核对（100%），实时脉络保有提示缓存。推荐的入门配置。 |
