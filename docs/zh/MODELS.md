...
- **30B 级是结构化/图谱抽取的下限** — 这不是我们的偏好,而是行业观察:
  ...
  知识图谱——完整配置的主模型正是这样选出来的，64 GB 上依然是 35B。但作为
  「不可低于」的硬性下限，这条规则并未成立：我们自己 8 月 19 日在三场真实
  会议上的基准显示，`qwen3.5:4b` 抽取图谱优于 `gemma4:12b`（31 条决定对 28，
  96% 对 100% 可核对引用），而破坏 JSON 的是具体模型（`GigaChat3.1-10B`，
  六分之五的分片），不是参数量级。详见下文「30B 下限已废除」。
...
因此在 8 GB 与 16 GB 配置上标注触发得更少——这是轻量配置的代价，而不是故障。
在 32 GB 与 64 GB 上，配置会把这里设为本就常驻的主模型。

**30B 下限已废除（8 月 19 日基准）。** 同一个 `scripts/bench_extract.py`
在三场真实会议（2.4 万–10.6 万字符）上的结果：`qwen3.5:4b` — 3/3 解析成功，
31 条决定、30 个核心、96% 引用，中位数 113 秒；`gemma4:12b` — 3/3，28 条决定、
24 个核心、100% 引用，353 秒；`gemma4:latest`（即 e4b）— 3/3，16 条决定、
23 个核心、95%，152 秒；`GigaChat3.1-10B` — 2/3，六分之五的分片 JSON 损坏。
