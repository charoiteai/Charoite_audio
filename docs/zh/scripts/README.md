...
- `dedup_graph.py` — 两条规则，各自需要单独许可：`--apply-copies` / `sufler.dedup_copies` 把 iCloud 冲突副本（与 `Имя.md` 逐字节相同的 `Имя 2.md`）移出图谱，放入 `backups/<图谱>/dedup_copies/` 并写清单；`--apply` / `sufler.dedup_files` 把逐字节相同的归档副本换成硬链接。不带参数时只输出报告。
