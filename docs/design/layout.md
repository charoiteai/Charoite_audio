...
- `scripts/layout_map.py`, форма «file»: сам гейт раскладки: путь к репозиторию нужен ему ДО того, как канон станет импортируемым — эта строка и добавляет src/ в sys.path. Канон здесь недостижим по порядку загрузки, а не по решению. Цепочка ведёт к корню КОДА (docs/design, src/), данных он не касается (№338)
