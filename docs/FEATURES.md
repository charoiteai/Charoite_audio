...
- **Live names** — once someone introduces themselves (or is addressed and
  ...
  person node. The rebuild after the meeting runs the model's answers through
  the same checks — a name heard only as an address («Тань, глянь…») is
  accepted in the nominative only when it came from the other side — and
  minutes regenerated by an earlier rebuild keep their own numbering: live
  session names are not stamped onto them.
...
- **The meeting card** — a ready meeting opens on click: topic, date,
  ...
  bridge mark is dropped, keyed after the bridge's `_key` and wider by the withdrawal marks — is hidden when the
...
stays as it is: done `[x]`, cancelled `[-]` (the Obsidian Tasks form), an item reopened by
hand, and any other mark in the box (`[/]`, theme letters like `[i]`). The output is checked,
not just the parser: a file whose rewrite would change a status is left alone and named with
its line number, and the script exits with 1. `--apply` needs `CHAROITE_ROOT`, writes under the
shared graph lock and keeps the originals plus a manifest in the graph backups, outside the
synced folder.
