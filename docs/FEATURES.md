...
2. **Graph update** — entities (People/Systems), decisions, action items,
   ...
   the node stays readable. A system or team node is not created when the
   name is not a name (a pronoun, a fragment), is one letter away from a
   node in the same folder, or matches several nodes: the name stays as text
   in the meeting note with links to the candidates, and the candidate goes
   to `logs/graph_ambiguous.md` — confirm with an alias in the node or create
   the node by hand.
