...
struct ExternalRecordingView: View {
    ...
    private var footer: some View {
        Text(L.t("Файл копируется в папку импорта — оригинал остаётся у вас. Обработанная копия живёт два дня (audio.import_keep_days) и удаляется вместе с аудио-исходником в архиве встречи; при ошибке ничего не удаляется. Всё локально.",
                 "The file is copied into the import folder — your original stays put. A processed copy lives two days (audio.import_keep_days) and is deleted together with the audio source in the meeting archive; on failure nothing is deleted. Everything stays local.",
                 "文件会复制到导入文件夹，你的原件保持不动。已处理的副本保留两天（audio.import_keep_days），随后与会议归档中的音频源一起删除；处理失败时不删除任何内容。全部本地完成。"))
            .font(.caption)
            .foregroundStyle(.secondary)
    }
