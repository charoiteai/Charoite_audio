...
enum ModelPresetPolicy {
    ...
    static let all: [ModelPreset] = [
        ...
        //
        // Вторая сверка (08.08, сплошной аудит документации): лёгкая модель
        // была qwen3.5:2b, а README и MODELS.md обещают на 8 ГБ ОДНУ модель
        // на обе роли. Обещание документации точнее: 3.4 + 1.8 ГБ резидентно
        // рядом с системой — это те же 8 ГБ впритык, ради которых мы и
        // убирали gemma. Одна модель обслуживает и подсказки, и тезисы.
        ModelPreset(
            id: "light",
            ...
            smallModel: "qwen3.5:4b",
            needsGB: 8,
            note: L.t("Для 8 ГБ: одна модель на обе роли. Граф знаний выключен.",
                      "For 8 GB: one model for both roles. Knowledge graph off.",
                      "适用于 8 GB：一个模型兼顾两种角色。知识图谱关闭。")),
    ]
