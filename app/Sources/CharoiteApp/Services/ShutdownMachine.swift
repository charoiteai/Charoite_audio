...
enum ShutdownMachine {
    ...
    static func next(_ phase: ShutdownPhase, on event: ShutdownEvent,
        ...
        // --- начало остановки
        //
        // Живой демон захват НЕ закрывает: ему дают доработать — записать
        // хвост аудио и запустить разбор встречи. Закрытие приходит позже,
        // от `daemonExited` или от страховочного таймера. Пока машина
        // утверждала обратное, модель расходилась с реальным путём, и это
        // стоило дефекта: сервис не мог прогнать событие через машину, не
        // закрыв захват раньше времени (ревью 19.08, третий круг).
        case (.idle, .stopRequested(let alive)):
            return alive ? (.waitingDaemon(waits: 0), .nothing) : (.done, .finish)

        // Повторный Стоп во время обычного ожидания ничего не меняет: процесс
        // уже добивается по расписанию. Но если он к этому моменту УМЕР —
        // закрываем встречу сразу, как и в фазе застревания: иначе контракт
        // машины непоследователен, а закрытие ждёт следующего опроса
        // (ревью 19.08, четвёртый круг).
        case (.waitingDaemon, .stopRequested(let alive)):
            return alive ? (phase, .nothing) : (.done, .finish)
