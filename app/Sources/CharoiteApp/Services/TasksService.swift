...
final class TasksService: ObservableObject {
    ...
    /// окна задач и после каждой отметки.
    func rescan() {
        guard let graph = AppSettings.graphDir else { items = []; openCount = 0; return }
        var found: [Item] = []
    ...
    func toggle(_ item: Item) {
        ...
        guard item.lineIndex < lines.count else { rescan(); return }
        ...
        guard flipped != line else { rescan(); return }
