...
Если на машине поднят опциональный brain-сервер (порт 8100), поиск
уходит в него: там тот же алгоритм плюс семантический слой (bge-m3),
который находит ответ даже без совпадения слов. Без него всё работает
локальным поиском по файлам.

Приложение не открывает сетевых соединений, кроме вашего Ollama
(и localhost-brain, если он поднят).
