...
def test_more_review_forms():
    ...
    assert rb.recovered_items(bold_item) == ["**Иван** — согласовать бюджет"], "строка с жирного без маркера — не хвост чужого пункта"
...
def test_round4_edges():
    ...
    assert rb.recovered_items(wrapped) == ["**Иван** — согласовать бюджет **на стенд** до пятницы"]
    ...
    _, a = rb.merge_into_minutes("## Поручения\n- нет\n", long_first)
    _, b = rb.merge_into_minutes("## Поручения\n- нет\n", list(reversed(long_first)))
    assert a == b == 1
