...
def test_canon_bases_unrolls_chains_keeps_namesakes_and_survives_cycles():
    ...
    keep = gs.canon_bases([gs.Doc("", "Досье/отчёт.md", 0.0, "", "", 0.0, "отчёт", "", "ядра/сводка"),
                           d("сводка"), d("отчёт")])
    assert keep == {"досье/отчет": "ядра/сводка"}, keep
    # а живой файл ПО ТОМУ ЖЕ ключу заглушку по-прежнему перебивает
    same = gs.canon_bases([gs.Doc("", "Ядра/отчёт.md", 0.0, "", "", 0.0, "отчёт", "", "ядра/сводка"),
                           d("сводка"), d("отчёт")])
    assert same == {}, same
    assert gs.canon_bases([d("а", "б"), d("б", "а")]) == {}, "цикл заглушек не переписывает ключи"
    ...
    assert gs.canon_bases([loop, real, alive]) == {"ядра/отчет": "ядра/итоги"}
    assert gs.canon_bases([real, loop, alive]) == {"ядра/отчет": "ядра/итоги"}, "исход решил обход"
    ...
    assert gs.canon_bases([in_cores, in_dossier, *both]) == expect
    assert gs.canon_bases([in_dossier, in_cores, *both]) == expect, "исход решил порядок обхода"
    ...
    assert gs.canon_bases([dead, in_cores, d("архив")]) == {"ядра/отчет": "ядра/архив"}
    ...
    assert gs.canon_bases([old_early, new_late, d("итоги"), d("архив")]) == {"ядра/отчет": "ядра/итоги"}
    ...
    y_dead = gs.Doc("", "Ядра/игрек.md", 0.0, "", "", 300.0, "игрек", "", "ядра/исчез")
    y_live = gs.Doc("", "Ядра/игрек.md", 0.0, "", "", 100.0, "игрек", "", "ядра/итоги")
    assert gs.canon_bases([x, y_dead, y_live, d("итоги")])["ядра/икс"] == "ядра/итоги"
