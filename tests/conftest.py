...
def _data_root_in_tmp(tmp_path, monkeypatch, request):
    ...
    if mod is not None and свой_корень:
        # Не подменяем, но и не доверяем на слово: сторож поверх настоящего
        # `_root` падает, если помеченный тест забыл назвать корень и ревизия
        # собралась писать снимки в корень установки — ровно те 69 каталогов
        # «Работа-*», из-за которых заведена эта фикстура (DS I2 круга 6).
        настоящий = mod._root

        def _сторож() -> pathlib.Path:
            корень = настоящий()
            assert tmp_path in корень.parents or корень == tmp_path, (
                f"тест с маркером настоящий_корень_ревизии не назвал корень: "
                f"ревизия собралась писать в {корень}")
            return корень

        monkeypatch.setattr(mod, "_root", _сторож)
