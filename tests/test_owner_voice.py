...
def test_эхо_липкое_и_после_затухания_динамиков():
    """Голос, хоть раз звучавший в динамиках, остаётся эхом и когда счётчик
    bh растаял до нуля: иначе после паузы собеседник из динамиков получал
    имя владельца (баг 20.08 у owner_voices, у owner_voice фильтра не было —
    аудит 13.09, GLM M1)."""
    heard = _call(mic={3: 50.0}, bh={3: 40.0})
    assert 3 in heard.echoed
    heard.bh[3] = 0.0                       # динамики давно молчат
    assert ov.owner_voice(heard) is None
...
def test_ровно_порог_речи_уже_достаточно():
    """Граница, которую не проверял ни один тест.

    Найдено мутационным прогоном 20.08: замена `<` на `<=` в проверке порога
    не убивала ни один тест — значит поведение РОВНО на пороге не держал
    никто. Разница не теоретическая: при `<=` владелец с ровно пятнадцатью
    секундами речи остаётся «Собеседником» до следующей реплики, а на
    коротком созвоне следующей может и не быть.
    """
    heard = _call(mic={7: ov.MIN_MIC_SECONDS})

    assert sum(heard.mic.values()) == ov.MIN_MIC_SECONDS, "тест не на границе"
    assert ov.owner_voices(heard) == {7}, "ровно порог — это уже достаточно"


def test_чуть_меньше_порога_ещё_рано():
    """Обратная сторона границы: случайный кашель не подписываем."""
