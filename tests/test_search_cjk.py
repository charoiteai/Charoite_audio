...
def test_гейт_и_счёт_согласованы_на_краевом_регистре():
    """IGNORECASE шире lower() на ς/σ и İ/i. После снятия re.I гейт стал
    строже — ровно настолько, насколько строг подсчёт очков: раньше такой
    файл проходил гейт и выводился со счётом 0 (замер 20.08)."""
    assert memory_bench.norm("ς") != memory_bench.norm("σ")
    # lower() в Python ставит конечную сигму по позиции: «ΟΔΟΣ» → «οδος».
    assert memory_bench.norm("ΟΔΟΣ") == "οδος"
