...
Проверка — записывающим шпионом, а не падающей подменой: сторож сети в conftest бросал
AssertionError, пути внешней памяти ловят Exception, и «не упало» ничего не доказывало бы
(входной круг, Opus C4). С №376 сторож роняет тест через `pytest.fail`, но шпион остаётся:
он говорит, что и куда отправлено, а не только что сеть не тронута.
"""
