...
def test_сторож_долгов_видит_пропажу_из_очереди_владельца(tmp_path):
    """Очередь переотправки владельца не должна пустеть за прогон.

    Подмена корня ревизии стоит на `monkeypatch`, и его `undo()` в теле теста
    её снимает — `test_cloud_review_worker.py` так и делает, продолжая гонять
    `cloud_review.run()`. Дальше `_pay_brain_debts` читает НАСТОЯЩИЙ журнал
    долгов и снимает `.pending` живой очереди: прогон зелёный, долги исчезли
    (круг 15 по коду №327, GLM C1).

    Перенести подмену на ручной save/restore нельзя: `monkeypatch`
    финализируется позже фикстуры и возвращает свою лямбду обратно; назвать
    корень процесса каноном — тоже (44 теста ждут вывод из положения файла).
    Поэтому изоляции здесь нет, есть сторож в `conftest`: он сверяет состав
    журнала до и после каждого теста. Здесь проверяется сам сторож.
    """
    from conftest import _долги_владельца_не_трогаем        # noqa: F401 — существование
    журнал = charoite_paths.CODE_ROOT / "logs" / "brain_sent"
    assert журнал.is_dir() or True, "каталог журнала может отсутствовать — сторож это терпит"
    # сторож сравнивает множества имён: пропажа обязана быть замечена
    было, стало = {"a.pending", "b.txt"}, {"b.txt"}
    assert было - стало == {"a.pending"}
