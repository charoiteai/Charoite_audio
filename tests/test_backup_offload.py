...
def test_сторож_долгов_видит_пропажу_и_терпит_отсутствие_каталога(tmp_path):
    """Отрицательный тест сторожа: пропажа замечена, отсутствие каталога — нет.

    Сторож — гейт безопасности данных владельца (живые `.pending` очереди
    переотправки), и без такой проверки он умрёт молча. Проверяется его
    настоящая работа: чтение каталога и сравнение состава, а не арифметика
    множеств (круг 16 по коду №327, GLM C1).
    """
    from conftest import очередь_долгов
    журнал = tmp_path / "brain_sent"
    assert очередь_долгов(журнал) == set(), "каталога нет — сторож молчит, а не падает"
    журнал.mkdir()
    (журнал / "2026-08-01_0900.txt").write_text("", encoding="utf-8")
    (журнал / "2026-08-01_0900.pending").write_text("", encoding="utf-8")
    было = очередь_долгов(журнал)
    assert было == {"2026-08-01_0900.txt", "2026-08-01_0900.pending"}
    (журнал / "2026-08-01_0900.pending").unlink()
    assert было - очередь_долгов(журнал) == {"2026-08-01_0900.pending"}, \
        "сторож не заметил пропажу долга — прогон гасил бы очередь владельца молча"
