...
def test_предусловие_маркера_ловит_корень_вне_прогона(tmp_path):
    """Сторож маркера — сам под тестом, иначе он умрёт молча.

    Нарушение одно и то же по сути: корень процесса оказался вне базового
    каталога прогона — будь то корень установки (снимки осели бы в
    репозитории) или живой корень данных владельца (ревизия прошлась бы по
    настоящим встречам). Законен сосед папки теста: `tmp_path_factory`
    раздаёт именно соседей (DS I1 круга 8).
    """
    from conftest import нарушение_маркера
    база = tmp_path.parent
    assert "вне временного каталога" in нарушение_маркера(pathlib.Path("/установка"), база)
    assert "вне временного каталога" in нарушение_маркера(pathlib.Path.home() / "Documents" / "Чароит", база)
    assert нарушение_маркера(tmp_path, база) == ""
    assert нарушение_маркера(база / "сосед", база) == ""
...
def test_помеченный_тест_не_теряет_изоляцию_графа(tmp_path):
    ...
    graph = os.environ.get("CHAROITE_GRAPH_DIR")
    assert graph and pathlib.Path(graph).is_relative_to(tmp_path), graph
