...
@pytest.mark.сеть_разрешена
def test_llm_под_открытой_сетью_падает_на_входе():
    """`LLM`, созданный под `сеть_разрешена`, ушёл бы в живую сеть: маршрутов у
    настоящего транспорта нет. Обёртка конструктора не молчит, а падает на
    создании (Important DeepSeek по PR #636). В сеть тест не ходит."""
    from llm import LLM
    with pytest.raises(pytest.fail.Exception, match="под сеть_разрешена создан LLM"):
        LLM(_cfg_ollama("http://127.0.0.1:11434"))
