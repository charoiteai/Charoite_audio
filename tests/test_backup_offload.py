...
def test_сторож_долгов_видит_пропажу_из_очереди_владельца(tmp_path):
    ...
    Починено в самом тесте (круг 16, DS C1): подмена моста живёт в своём
    `pytest.MonkeyPatch.context()` и общий `undo()` больше не нужен. Сторож
    остаётся вторым рубежом — на случай следующего теста, который позовёт
    `undo()` посреди работы. Здесь проверяется сам сторож.
    """
