...
def test_модуль_отвечает_текущим_корнем_а_не_замороженным(tmp_path):
    ...
    уровне: значение снималось на СБОРКЕ, раньше любой фикстуры и раньше, чем
    точка входа успевала назвать корень. Процесс разъезжался сам с собой —
    половина модулей в названном корне, половина в выведенном из положения
    файла, молча (замер 21.09, №329; до этого обвязка была вынуждена называть
    корень до сборки, чтобы снимок хотя бы замораживал временный каталог).

    Здесь проверяется противоположное свойство: заморозки больше нет. Снимите
    ленивый вызов, вернув модульную константу, — тест покраснеет.
    """
    прежний = daemon._root()
    названный = charoite_paths.use_data_root(tmp_path / "новый-корень", replace=True)
    assert названный != прежний, "предпосылка: новый корень отличается от прежнего"
    for модуль in (daemon, meeting_archive):
        assert модуль._root() == названный, (
            f"{модуль.__name__} держит замороженный корень: {модуль._root()}")
