...
def проверить_изоляцию_графа(tmp_path) -> None:
    """Граф процесса лежит внутри каталога теста — одна форма на всех.
    ...
    for имя in ("CHAROITE_GRAPH_DIR", "SUFLER_GRAPH_DIR"):
        ...
        assert значение, f"{имя} снят — изоляции графа нет"
        assert pathlib.Path(значение).is_relative_to(tmp_path), \
            f"{имя}={значение} вне каталога теста — тест пойдёт в граф владельца"
...
def test_помеченный_тест_не_теряет_изоляцию_графа(tmp_path):
    ...
    проверить_изоляцию_графа(tmp_path)
