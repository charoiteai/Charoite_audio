...
def test_предусловие_маркера_ловит_оба_нарушения(tmp_path):
    """Сторож маркера — сам под тестом, иначе он умрёт молча.

    Два нарушения: корень не назван вовсе и корень назван внутри установки.
    Законный случай — любой путь вне установки, включая соседа `tmp_path`
    (`tmp_path_factory.mktemp()` даёт именно соседа, и привязка к `tmp_path`
    объявляла бы его нарушением; DS I1 круга 7).
    """
    from conftest import нарушение_маркера
    установка = pathlib.Path("/установка")
    assert "не назвал корень" in нарушение_маркера(None, установка)
    assert "внутри установки" in нарушение_маркера(установка / "src", установка)
    assert "внутри установки" in нарушение_маркера(установка, установка)
    assert нарушение_маркера(tmp_path.parent / "сосед", установка) == ""
