...
#: Модули, у которых корень данных больше не снимается на импорте. Список
#: рядом со свидетелем: проверять два из тринадцати значит оставить одиннадцать
#: без той самой проверки, ради которой правка делалась (круг 1 по коду №329,
#: GLM I2). Имя спрашивающего — у каждого своё, поэтому пара (модуль, как
#: спросить).
СПРОСИТЬ_КОРЕНЬ = (
    ("audio", lambda м: м._root()),
    ("daemon", lambda м: м._root()),
    ("diarize", lambda м: м._root()),
    ("dictate", lambda м: м._root()),
    ("dictate_note", lambda м: м._root()),
    ("llm_health", lambda м: м._root()),
    ("main", lambda м: м._root()),
    ("mcp_server", lambda м: м._root()),
    ("meeting_archive", lambda м: м._root()),
    ("rebuild_transcript", lambda м: м._root()),
    ("retro_fill", lambda м: м._root()),
    ("transcribe_file", lambda м: м._root()),
    ("graph_updater", lambda м: м._root()),
    ("nli", lambda м: м._dir().parent.parent),          # путь к модели производный
    ("llm", lambda м: м.LLM.__new__(м.LLM)._root()),    # поле класса тоже шло на импорте
)
...
def test_модуль_отвечает_текущим_корнем_а_не_замороженным(tmp_path):
    ...
    for имя, спросить in СПРОСИТЬ_КОРЕНЬ:
