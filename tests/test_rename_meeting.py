...
def test_legacy_main_with_an_aux_tail_is_healed_by_rename(tmp_path):
    ...
    p = rm.plan(tmp_path / "нет-графа", tmp_path, STAMP, "Демо live", rm.pretty_and_slug("Демо live")[1])
    moves = {src.name: dst.name for src, dst in p["moves"]}
    assert moves[main.name] == f"{STAMP}_Демо-live.md"
    assert moves[f"{STAMP}_Демо_live_minutes.md"] == f"{STAMP}_Демо-live_minutes.md"
    assert moves[copy.name] == f"{STAMP}_Демо-live_live.md"
