...
def test_в_тестах_нет_двух_определений_с_одним_именем():
    """Два `def` с одним именем в модуле — тихо выключённый тест.

    pytest собирает тесты как атрибуты модуля: от двух определений остаётся
    последнее, первое не исполняется никогда. Правка «дописать рядом» вместо
    «заменить» так и выглядит — зелёный прогон, счёт тестов не меняется, а
    проверок нет. Именно это случилось с двумя тестами диктовки: три находки
    круга 13 считались закрытыми, пока их не нашла голова круга 14
    (№327, DS C1).
    """
    дубли = {}
    for файл in sorted(pathlib.Path(__file__).resolve().parent.glob("test_*.py")):
        имена = [узел.name for узел in ast.parse(файл.read_text(encoding="utf-8")).body
                 if isinstance(узел, (ast.FunctionDef, ast.AsyncFunctionDef))
                 and узел.name.startswith("test_")]
        повторы = {имя for имя in имена if имена.count(имя) > 1}
        if повторы:
            дубли[файл.name] = sorted(повторы)
    assert not дубли, f"тесты с одинаковыми именами — работает только последний: {дубли}"
