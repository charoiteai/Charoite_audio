...
def test_owner_is_written_one_way_in_the_assignments_section():
    """№188: имя владельца в дательном падеже или через «е» вместо «ё» — первым
    словом user_name, чтобы окно «Задачи» узнало его целым словом. Фамилия,
    уменьшительные, составные и чужие исполнители, другие разделы и пустое
    имя — как есть: цена ошибки здесь «чужая задача сменила исполнителя»."""
    from action_items import canon_owner, canon_owner_item
    owner = "Марк Ковалёв"
    doc = ("## Поручения\n- [ ] **Марку** — смета\n- [x] **марку** — регистр\n- [ ] **МАРК** — верхний регистр\n"
           "- [ ] **Ковалёву** — фамилия в падеже\n- [ ] **Ковалев** — фамилия без ё\n"
           "- [ ] **Марк Ковалёв** — полное имя\n- [ ] **Ковалёв** — фамилия\n"
           "- [ ] **Маркус** — чужой\n- [ ] **Марк и Инга** — двое\n- [ ] **Марку Ковалёву** — два слова\n"
           "- [ ] **Команда** — все\n## Открытые вопросы\n- [ ] **Марку** — вне раздела\n")
    out = canon_owner(doc, owner)
    assert out.split("\n")[1:12] == [
        "- [ ] **Марк** — смета", "- [x] **Марк** — регистр", "- [ ] **Марк** — верхний регистр",
        "- [ ] **Ковалёв** — фамилия в падеже", "- [ ] **Ковалёв** — фамилия без ё",
        "- [ ] **Марк Ковалёв** — полное имя", "- [ ] **Ковалёв** — фамилия", "- [ ] **Маркус** — чужой",
        "- [ ] **Марк и Инга** — двое", "- [ ] **Марку Ковалёву** — два слова", "- [ ] **Команда** — все"], out
    ...
    assert canon_owner_item("**Ивану** — X", "") == "**Ивану** — X", "пустой user_name — как есть"
    # формы выводятся из слова user_name: дательный по типу склонения, беглая
    # гласная и «Любовь» таблицей, ё/е и регистр в обе стороны
    yes = [("**Петру** — письмо", "Пётр Иванов", "Пётр"), ("**Петр** — письмо", "Пётр Иванов", "Пётр"),
           ("**Пётр** — письмо", "Петр Иванов", "Петр"), ("**Семёну** — письмо", "Семен Смирнов", "Семен"),
           ("**Павлу** — смета", "Павел Иванов", "Павел"), ("**Льву** — смета", "Лев Иванов", "Лев"),
           ("**Игорю** — смета", "Игорь Петрович Ветров", "Игорь"), ("**Андрею** — смета", "Андрей", "Андрей"),
           ("**Анне** — отчёт", "Анна Орлова", "Анна"), ("**Марии** — отчёт", "Мария Орлова", "Мария"),
           ("**Наталье** — отчёт", "Наталья", "Наталья"), ("**Илье** — отчёт", "Илья Орлов", "Илья"),
           ("**Любови** — отчёт", "Любовь Никитина", "Любовь"),
           ("**Александре** — макет", "Александра Иванова", "Александра"),
           ("**Ветрову** — дело", "Игорь Ветров", "Ветров")]        # фамилия → фамилия, не имя
    for item, who, want in yes:
        assert canon_owner_item(item, who) == f"**{want}**" + item[item.index("**", 2) + 2:], (item, who)
    # Critical круга 1 по #536: чужое имя, похожее на владельца, — НЕ владелец
    no = [("**Александре** — макет", "Александр Ковалёв"),     # дательный ЖЕНСКОГО имени
          ("**Александру** — макет", "Александра Иванова"),    # и наоборот
          ("**Вите** — макет", "Виктор Смирнов"),             # уменьшительное/чужая Вита
          ("**Ковалёвой** — документы", "Марк Ковалёв"),       # фамилия другого пола
          ("**Вере** — отчёт", "Вероника Иванова"),            # «Вера» в таблице уменьшительных
          ("**Ярославу** — отчёт", "Слава Петров"),            # многозначное уменьшительное
          ("**Евгении** — отчёт", "Евгений Петров"),
          ("**Данилу** — отчёт", "Даниел Иванов"),             # круг 2: правило «-ел» давало дательный Данила
          ("**Саша** — позвонить", "Александр Ковалёв"),       # уменьшительные не трогаем
          ("**Марина** — отчёт", owner), ("**Марком** — отчёт", owner),
          ("**Марк Чужой** — встреча", owner), ("**Игорь Ветров** — дело", "Игорь Петрович Ветров"),
          ("позвонить Марку — до пятницы", owner)]             # имя не в позиции ответственного
    for item, who in no:
        assert canon_owner_item(item, who) == item, (item, who)


def test_finalize_assignees_canonizes_the_owner_before_marking_outsiders():
    """Critical DS r2 по #536: «Павлу» у владельца-Павла flag_outsiders участником
    не считает (_same_person не знает беглой гласной) — при порядке «пометка →
    канон» владелец терял собственную задачу вместе с чекбоксом. Порядок —
    контракт finalize_assignees, и она одна на все пути записи."""
    from action_items import finalize_assignees
    doc = "## Поручения\n- [ ] **Павлу** — смета\n- [ ] **Льву** — макет\n- [ ] **Пётр** — письмо\n"
    out = finalize_assignees(doc, {"Павел Иванов", "Пётр Сидоров"}, "Павел Иванов")
    assert out.split("\n")[1:4] == ["- [ ] **Павел** — смета",
                                    f"- {OUTSIDER_MARK} (Льву): **Льву** — макет",
                                    "- [ ] **Пётр** — письмо"], out
    # обратный порядок — та самая потеря: чекбокс снят, канон строку уже не видит
    assert f"{OUTSIDER_MARK} (Павлу)" in flag_outsiders(doc, {"Павел Иванов", "Пётр Сидоров"})
