...
def test_owner_is_written_one_way_in_the_assignments_section():
    ...
    yes = [("**Петру** — письмо", "Пётр Иванов", "Пётр"), ("**Петр** — письмо", "Пётр Иванов", "Пётр"),
           ...
           ("**Любови** — отчёт", "Любовь Никитина", "Любовь"), ("**Нинели** — отчёт", "Нинель Орлова", "Нинель"),
           ("**Александре** — макет", "Александра Иванова", "Александра"),
           ("**Ветрову** — дело", "Игорь Ветров", "Ветров"),         # фамилия → фамилия, не имя
           ("**Ковалёвой** — смета", "Анна Ковалёва", "Ковалёва"),   # женская фамилия (DS r3)
           ("**Ковальскому** — смета", "Марк Ковальский", "Ковальский"),
           ("**Ковальской** — смета", "Анна Ковальская", "Ковальская"),
           ("**Роману** — дело", "Иван Роман", "Роман")]              # фамилия как имя — принятая коллизия
...
def test_finalize_assignees_canonizes_the_owner_before_marking_outsiders():
    ...
    # …а участники из participants_set несут формы владельца сами: легаси-строку
    # «**Павлу**» в минутках флаг без канона не помечает (Important GLM r3)
    parts = participants_set(["Пётр Сидоров"], owner="Павел Иванов")
    assert {"Павел Иванов", "Пётр Сидоров", "павлу", "павел", "иванову"} <= parts, parts
    assert flag_outsiders(doc, parts).split("\n")[1] == "- [ ] **Павлу** — смета"
    # один участник — формы не добавляются: гейт «судить некого» считает строки
    assert participants_set(["Собеседник 2"], owner="Павел Иванов") == {"Павел Иванов"}
...
def test_daemon_uses_transcript_participants_not_full_text(tmp_path):
    ...
    assert {"Андрей", "Дмитрий", "Аня", "Игорь Ветров"} <= got, got
    assert got - {"Андрей", "Дмитрий", "Аня", "Игорь Ветров"} == {"игорь", "игорю", "ветров", "ветрову"}, \
        "остальное — только формы владельца (№188)"
