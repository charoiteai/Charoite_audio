...
def test_people_description_is_superseded_with_dates_not_silently(tmp_path):
    ...
    # пересказ: перестановка слов и более короткая форма — не новый факт
    g.upsert_entity(graph, "Люди", "Пётр", "person", "руководитель миграции проекта", "Встречи/2026-08-22_1000", "")
    g.upsert_entity(graph, "Люди", "Пётр", "person", "руководитель проекта", "Встречи/2026-08-23_1000", "")
    text = node.read_text(encoding="utf-8")
...
def test_same_fact_rule_table():
    """Одно правило (круг 2 по #539, второй Critical в той же правке — упрощаем):
    пересказ = те же слова короче или в другом порядке при тех же числах и
    словах-переворотах; всё остальное — новый факт."""
    same = g._same_fact
    for old, new in [("аналитик", "Аналитик"), ("ведёт отчётность", "ведет отчетность"),
                     ...
                     ("руководитель проекта миграции", "руководитель проекта"),
    ...
    for old, new in [("не участвует в проекте", "участвует в проекте"),      # снятое отрицание (Critical r2)
                     ...
                     ("QA", "руководитель QA-направления"), ("", "аналитик"), ("аналитик", "")]:
...
def test_description_supersede_edges(tmp_path):
    ...
    # прочерки любого вида — не описание (DS r2)
