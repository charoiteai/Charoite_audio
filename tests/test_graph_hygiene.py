...
def test_people_description_is_superseded_with_dates_not_silently(tmp_path):
    ...
    с обеими формулировками и датой встречи, как статус у Ядер; пересказ того
    же (вложение, общие слова) новым фактом не считается; пустое не вытесняет;
    ретрай той же встречи строк не плодит; пустой узел заполняется без следа."""
    graph = tmp_path / "g"
    (graph / "Люди").mkdir(parents=True)
    g.upsert_entity(graph, "Люди", "Пётр", "person", "аналитик", "Встречи/2026-07-15_1400", "спросил")
    node = graph / "Люди" / "Пётр.md"
    g.upsert_entity(graph, "Люди", "Пётр", "person", "аналитик данных", "Встречи/2026-07-20_1000", "")
    text = node.read_text(encoding="utf-8")
    assert "# Пётр\nаналитик\n" in text and "## Хроника" not in text, "пересказ — не новый факт"
    ...
    # второе вытеснение — сверху хроники, прежняя строка остаётся
    g.upsert_entity(graph, "Люди", "Пётр", "person", "перешёл в отдел данных", "Встречи/2026-09-01_1000", "")
    text = node.read_text(encoding="utf-8")
    assert text.count("_(было:") == 2 and text.index("2026-09-01_1000") < text.index("2026-08-20_1000")
    assert "было: «руководитель проекта миграции» → стало: «перешёл в отдел данных», 2026-09-01" in text
    # узел без описания получает первое без строки хроники — это не вытеснение
    old = graph / "Люди" / "Старый.md"
    old.write_text("# Старый\n\n## Встречи\n- [[Встречи/2026-05-05_1000]]\n", encoding="utf-8")
    ...
    # описание из нескольких строк заменяется целиком, служебная строка не задета
    ...
    assert "# Многострочный\nсовсем новая роль\n\n_(последнее упоминание: 2026-08-02)_" in t
    assert "было: «строка раз строка два»" in t and "строка два\n" not in t.split("## Хроника")[0]
