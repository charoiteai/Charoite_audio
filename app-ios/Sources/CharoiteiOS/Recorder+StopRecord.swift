...
extension Recorder {
    ...
    enum StopReason: String, Codable, CaseIterable, Sendable {
        ...
        /// Строка для экрана — производная значения, не наоборот. Терминальный стоп
        /// и ротация одной причины звучат по-разному: у ротации встреча продолжена
        /// новым файлом, у стопа — остановлена, поэтому исход — обязательный аргумент:
        /// безаргументного варианта нет, чтобы «забыть про ротацию» было нельзя
        /// синтаксически (Minor DS и GLM круга 2). Счёт сбоев кодека — из политики.
